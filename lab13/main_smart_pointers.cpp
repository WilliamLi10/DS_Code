...
class SharedPBalloon {
public:
    // CONSTRUCTOR & DESTRUCTOR
    SharedPBalloon(const std::string& name_) : name(name_) {
        std::cout << "Balloon constructor " << name << std::endl;
        num_ropes = 0;
    ...
    // add a rope connecting this balloon to another
    void addRope(dsSharedPtr<SharedPBalloon>& b) {
        assert (num_ropes < MAX_NUM_ROPES);
        ropes[num_ropes] = b;
        num_ropes++;
    }
    // detach a rope connecting this balloon to another
    void removeRope(dsSharedPtr<SharedPBalloon>& b) {
        for (int i = 0; i < MAX_NUM_ROPES; i++) {
            if (ropes[i] == b) { ropes[i] = ropes[num_ropes-1]; num_ropes--; break; }
        }
    }
...
private:
    ...
    int num_ropes;
    // a dynamically allocated C-style array of ropes connecting to other Balloons
    dsSharedPtr<SharedPBalloon> ropes[MAX_NUM_ROPES];
};
...
int main() {
  ...
  dsSharedPtr<SharedPBalloon> georgette(new SharedPBalloon("Mr Potato Head"));
    dsSharedPtr<SharedPBalloon> henry(new SharedPBalloon("Snoopy"));
    georgette->addRope(henry);
    georgette->print();
    henry->print();

    dsSharedPtr<SharedPBalloon> isabelle(new SharedPBalloon("Shrek"));
    henry->addRope(isabelle);
    isabelle = dsSharedPtr<SharedPBalloon>(new SharedPBalloon("Barney the Purple Dinosaur"));
